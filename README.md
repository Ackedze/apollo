...
При проверке внешнего компонента Apollo отдельно сравнивает каждый вложенный Contract v2 scope с variant-aware standalone reference его contract boundary. Штатные host overrides не подменяют этот baseline: они подавляются только через явные host-controlled policies для конкретных nested paths и свойств. Поэтому `BodyCell Wide / Presets=Account` не показывает разрешённые padding `PaymentMaskedNumber`, но изменение `Major.styles.text` остаётся нарушением относительно собственного контракта `PaymentMaskedNumber`. Evidence адресуется ID границы scope и не смешивается с baseline родительского `BodyCell` или промежуточного `Text`. Перед оценкой Apollo повторно собирает component keys из materialized snapshot и дожидается соответствующих v2-пакетов: nested-проверки не зависят от порядка предыдущих запусков и прогретого runtime-кэша. Поэтому правила `Amount` для item spacing, типографики, цвета и opacity одинаково работают при выборе `BodyCell Wide`, его `Text` и самого `Amount`; совпадающее evidence внутри одного scope дедуплицируется.
...
- Tree evaluator Contract v2 учитывает декларативный `facts.contractOwnership`: режим `host-contract` оставляет проверку вложенного package за host-компонентом. Для `AmountStyles → web-core.amount` это устраняет дубли цвета/типографики и сохраняет effective baseline выбранного `Style`; самостоятельный Core Amount продолжает проверяться своим package contract.
