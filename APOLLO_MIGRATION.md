...
- Локальные reference-артефакты сведены к [`JSONS`](./JSONS).

## Что ещё можно сделать при желании
1. Переименовать локальную папку проекта `Nemesis-mvp` в `Apollo` или `apollo`.
2. Обновить `AUDIT.md`, если нужен полностью актуальный технический отчёт уже без переходного контекста.
