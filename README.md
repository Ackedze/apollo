...
- В UI появился новый экспериментальный таб `Темизация`: он живёт рядом со старым `Ошибки темизации`, проверяет page-level mode `Theme / Corp` и использование `[Corporate]`-компонентов, чтобы новую эвристику можно было валидировать параллельно со старой.
