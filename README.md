...
- Декларативные composition rules хранятся вне plugin bundle в `Ackedze/design-system_ab/JSONS/apollo/patternRules.json`. Первый проверенный набор schema v1 содержит 24 правила для ButtonsGroup, BackgroundPlate, ContentCardWrapper, TitleView, Onboarding Hint/Tooltip, ButtonStack, Status/Property и secondary TabsView. [`patternRules.ts`](./src/assessment/patternRules.ts) содержит только типы, строгую валидацию и evaluator, поэтому изменение набора правил не требует пересборки Apollo. Контекстные зависимости от variant родителя, visibility, instance swap, count или order нельзя приближённо кодировать через v1: они остаются в component contracts до появления следующей версии схемы.
