...
Важно: strict comparison не создаёт warning для отсутствующего `radius`, если reference содержит эффективное значение `0`. Также отсутствие paint или padding у корня actual `INSTANCE`, сопоставленного с reference `COMPONENT`, считается ограничением Figma snapshot, а не ошибкой данных; доступное и реально изменённое свойство по-прежнему попадает в `Кастомизации`.
