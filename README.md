...
## GPT и тексты
В коде есть UI-блок `Проверка текстов через Yandex Cloud AI`, но в текущем состоянии его нельзя считать рабочей пользовательской функцией:

- `ui.html` умеет отправлять сообщение `gpt-request`;
- `code.ts` такое сообщение сейчас не обрабатывает;
- массивы текстовых узлов собираются во время аудита, но в `scan-result` отправляются как счётчики, а не как полный список.

Итог: текстовый/GPT-поток в репозитории присутствует частично и выглядит как незавершённая или деградировавшая функциональность.

## Ограничения и известные проблемы
- Плагин сканирует только видимые узлы: скрытые ветки отбрасываются ещё на этапе обхода.
- Кнопка в UI визуально переключается в состояние `Отменить`, но текущий обработчик не отправляет `cancel-scan`; фактической отмены из UI нет.
...
Подробный технический отчёт по найденным рискам хранится в [`AUDIT.md`](./AUDIT.md), но перед использованием стоит учитывать, что этот файл частично устарел и не полностью отражает текущее состояние GPT-потока.
